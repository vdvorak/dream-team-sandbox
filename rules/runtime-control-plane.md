...
- **Router** (`server/runtime/router.py`): nová route `GET /v1/environments/{project_id}/files`.
  Query params `path?: str`, `suffix?: str` (viz OpenAPI). Auth povinný.
  Mapuje na `service.list_files(project_id, path, suffix)`.
  Response schema = `FileList`.
- **Service** (`server/runtime/service.py`): `async def list_files(project_id, path, suffix)`:
  1. Prostředí musí být `ready` (shodně s RCP-A11).
  2. Sanitize `path` → **path sandbox check (load-bearing):**
     - Resolve absolutní cesta = `workspace_root / Path(path).resolve()`.
     - **MUST:** výsledná cesta MUSÍ začínat `workspace_root` (path assertion na `Path.resolve()`).
